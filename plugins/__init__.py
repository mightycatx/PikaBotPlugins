...
async def _mute(spdr):
    ...
    try:
        from pikabot.sql_helper.spam_mute_sql import mute
...
async def unmute(unmot):
    ...
    # Check if the function running under SQL mode
    try:
        from pikabot.sql_helper.spam_mute_sql import unmute
    except AttributeError:
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        if (
            ...
            if ((event.peer_id).user_id) == bot.uid:
               iname=ax[0]
            if bot2 is not None and ((event.peer_id).user_id) == bot2.uid
               iname=ax[1]
            if bot3 is not None and ((event.peer_id).user_id) == bot3.uid
               iname=ax[2]
            if bot4 is not None and ((event.peer_id).user_id) == bot4.uid
               iname=ax[3]
            reply_pop_up_alert = "Hi {}'s bot here ,\n\nWhy r u clicking this this.Please get your own PikaBot, and don't use mine!".format(iname)
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
